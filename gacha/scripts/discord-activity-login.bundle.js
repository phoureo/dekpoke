...
// ../../../discord/gacha/scripts/discord-activity-login.entry.js
...
async function loginWithDiscordActivity(options = {}) {
  const {
    clientId = "",
    tokenEndpoint = "",
    scopes = ["identify"],
    readyTimeoutMs = 15e3,
    authorizeTimeoutMs = 25e3,
    authenticateTimeoutMs = 25e3
  } = options;
  if (!clientId) {
  ...
  if (!tokenResponse.ok || !tokenPayload?.ok || !tokenPayload?.access_token) {
    throw new Error(
      String(
        tokenPayload?.message || tokenPayload?.error || `Discord Activity token exchange failed with HTTP ${tokenResponse.status}`
      )
    );
  }
